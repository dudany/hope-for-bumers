...
from pipelines.unemployment_etl.utils.unemployment_utils import set_logging, get_spark_session
...
def ingest_raw_data(raw_data_location: str, date_period: Dict[str, str], series_list: List[str]):
    ...
    spark_df = spark.createDataFrame(total_series_df)
    # repartition to reduce number of files and each json file per series_id
    spark.conf.set("mapreduce.fileoutputcommitter.marksuccessfuljobs", "false")  # to remove .crc files
